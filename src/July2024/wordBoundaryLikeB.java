...
public class wordBoundaryLikeB {
	...
	/*
	 The \b in regular expressions is a meta character that represents a word boundary. 
	 It is used to match the position where a word starts or ends. 
	 This is particularly useful when you need to match a word without matching substrings 
	 that may contain that word.
	 */
	...
	public static void boundaryWordMethodWithoutPatternAndMatcher()
	{
		try {
			...
			for(int i=0;i<arrayOfString.length;i++)
			{
				...
				|| (arrayOfString[i]>='A' && arrayOfString[i]<='Z')
				|| (arrayOfString[i]==' '))
				{
